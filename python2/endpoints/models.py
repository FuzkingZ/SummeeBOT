...
class Profile(models.Model):
    ...
    # Relationship Fields
    mid = models.ForeignKey('account',on_delete=models.CASCADE)
...
class account(models.Model):
    ...
    timeline_token = models.TextField(max_length=255 )# populate_from='timeline_token')
    qrcodeurl = models.URLField()
    mid = models.CharField(max_length=100)
    Name = models.TextField(max_length=255 )#, populate_from='name', blank=True )
...
class timeline_msg(models.Model):
    ...
    # Relationship Fields
    timelineid = models.ForeignKey('timeline',on_delete=models.CASCADE)
...
class message_msg(models.Model):
    ...
    # Relationship Fields
    msgid = models.ForeignKey('message',on_delete=models.CASCADE)
...
class message(models.Model):
    ...
    packageid = models.CharField(max_length=30, blank=True )
    stickerid = models.CharField(max_length=30, blank=True )
...
class timeline(models.Model):
    ...
    # Fields
    name = models.CharField(max_length=255 )#, populate_from='name', blank=True )
    slug = extension_fields.AutoSlugField(populate_from='slug', blank=True)
    created = models.DateTimeField(auto_now_add=True, editable=False)
    last_updated = models.DateTimeField(auto_now=True, editable=False)
    timelineid = models.AutoField(primary_key=True)
    text = models.TextField( )#populate_from='text',)
    picture = models.ImageField(upload_to="./upload/images/")
    stickerID = models.CharField(max_length=30 )#, populate_from='stkID', blank=True )
    packageID = models.CharField(max_length=30)#,populate_from='packID', blank=True )
